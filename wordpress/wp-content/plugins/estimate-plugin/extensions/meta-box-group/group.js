/* global jQuery, _, RWMB_Group */
( function( $, _, document, window, RWMB_Group ) {
	'use strict';
	...
	group.clone.remove = function( event ) {
		event.preventDefault();
		event.stopPropagation();
		var ok = confirm( RWMB_Group.confirmRemove );
		if ( ! ok ) {
			return;
		}
	...
	$( function() {
		...
		$wrapper.on( 'click', '.rwmb-group-title-wrapper, .rwmb-group-toggle-handle', group.toggle.handle );
		...
		$wrapper.on( 'click', '.rwmb-group-remove', group.clone.remove );
	} );
} )( jQuery, _, document, window, RWMB_Group );
