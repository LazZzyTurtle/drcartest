...
class DashboardEssential {
	...
	public function delete_api_key() {
		...
		if ( 200 !== (int) $response_code ) {
			return new \WP_Error( $response_code, __( 'HTTP Error', 'car-repair-services' ) );
		}
	...
	public function envato_theme_license_dashboard_update_message_cb( $plugin_data, $result ) {
		$status = get_option( 'envato_theme_license_key_status' );
		if ( $status != 'valid' ) {
			echo sprintf( __( 'To receive automatic updates license activation is required. Please visit <a href="%s">Setting</a> page.', 'car-repair-services' ), esc_url( admin_url() . 'admin.php?page=' . $this->menu_slug . 'product-registration' ) );
		}
	}
			...
			if ( $traker != '' ) {
				...
				if ( $status == 'valid' ) { ?>
					<div class="notice notice-success">
						<p><strong><?php esc_html_e( 'License Activated', 'car-repair-services' ); ?> </strong></p>
					</div>
				<?php } elseif ( $status == 'deactivated' ) { ?>
					<div class="notice notice-success">
						<p><strong><?php esc_html_e( 'License Deactiveted', 'car-repair-services' ); ?><strong></p>
					</div>
				<?php } else { ?>
					<div class="notice notice-error">
						<p><strong><?php echo sprintf( __( '%s', 'car-repair-services' ), $status ); ?><strong></p>
					</div>
					<?php
				}
			} else {
				...
				if ( $token != '' ) {
					?>
					<div class="notice notice-success">
						<p><strong><?php esc_html_e( 'License Key saved', 'car-repair-services' ); ?><strong></p>
					</div>
				<?php } else { ?>
					<div class="notice notice-error">
						<p><strong><?php esc_html_e( 'License Key blank', 'car-repair-services' ); ?><strong></p>
					</div>
					<?php
				}
			}
	...
	public function deactivated() {
		...
		if ( ! $url ) {
			$this->returnmassage = esc_html__( 'Api Key Not Found..', 'car-repair-services' );
			$status              = 'deactivated';
		} else {
			$args     = array(
				'timeout'   => 15,
			...
			if ( is_wp_error( $response ) ) {
				return new \WP_Error( $response_code, __( 'Error Found', 'car-repair-services' ) );
			}
			$license_data = json_decode( wp_remote_retrieve_body( $response ) );
			$status       = $license_data->status;
	...
	public function envato_theme_license_dashboard_sample_admin_notice() {
		...
		if ( $status != 'valid' || $purchase_key == '' || $this->token == '' ) {
			...
			<div id="setting-error-notice" class="error settings-error notice is-dismissible">
				<p><strong><span class="setting-error-notice-heading" style="margin-top:-0.4em"><?php echo esc_html__( 'Require Activation', 'car-repair-services' ); ?></span><span style="display: block; margin: 0.5em 0.5em 0 0; clear: both;"><?php echo sprintf( __( "%1\$s Theme Need to active with purchase code. Otherwise you can't Active / Update Bundle Plugin. You can active from <a href='%2\$s'>Here</a>.", 'car-repair-services' ), $this->dashboard_Name, esc_url( admin_url() . 'admin.php?page=' . $this->menu_slug . 'product-registration' ) ); ?> </span>
					</strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php echo esc_html__( 'Dismiss this notice.', 'car-repair-services' ); ?> </span></button>
			</div>
			<?php
		}
	...
	public function envato_theme_license_system_change_admin_notice() {
		...
		if ( $purchase_key != '' & $apikeys == '' ) {
			  $error_massage = esc_html__( 'Our theme license system has been changed, Please active again your theme license', 'car-repair-services' );
	...
	public function envato_theme_license_dashboard_style() {
		...
		wp_localize_script(
			...
			'ajax_dashboard_js',
			array(
				'copytext'          => esc_html__( 'Copied!', 'car-repair-services' ),
	...
	public function envato_theme_license_dashboard_get_tabs( $activetab ) {
		...
		$tabarray = array(
			'start'        => array(
				'title' => esc_html__( 'Getting Started', 'car-repair-services' ),
				'link'  => '?page=' . $this->menu_slug_dashboard,
			),
			'registration' => array(
				'title' => esc_html__( 'Registration', 'car-repair-services' ),
				'link'  => '?page=' . $this->menu_slug . 'product-registration',
			),
			'systemstatus' => array(
				'title' => esc_html__( 'System Status', 'car-repair-services' ),
				'link'  => '?page=' . $this->menu_slug . 'system-status',
			),
			'plugin'       => array(
				'title' => esc_html__( 'Plugins', 'car-repair-services' ),
		...
		if ( class_exists( 'OCDI_Plugin' ) && $this->token != '' ) {
			$tabarray['demo'] = array(
				'title' => esc_html__( 'Demo Import', 'car-repair-services' ),
		...
		if ( class_exists( 'iconmoonFontAdd' ) && $this->token != '' ) {
			$tabarray['icon'] = array(
				'title' => esc_html__( 'Icon Add', 'car-repair-services' ),
				'link'  => '?page=custom-icon-upload',
			);
		}
		$tabarray['support'] = array(
			'title' => esc_html__( 'Support', 'car-repair-services' ),
		...
		<h2 class="nav-tab-wrapper">
			...
			foreach ( $tabarray as $key => $tab ) {
				if ( $activetab == $key ) {
					?>
					<span class="nav-tab nav-tab-active"><?php echo sprintf( __( '%s', 'car-repair-services' ), $tab['title'] ); ?></span>
					<?php
				} else {
					?>
					<a href="<?php echo esc_url( $tab['link'] ); ?>" class="nav-tab"><?php echo sprintf( __( '%s', 'car-repair-services' ), $tab['title'] ); ?></a>
					<?php
				}
			}
			?>
