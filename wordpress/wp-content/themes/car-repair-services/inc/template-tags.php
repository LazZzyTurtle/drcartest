...
function car_repair_services_posted_on() {
	...
	$posted_on = sprintf(__( ' %s','car-repair-services'),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html__( 'by %s', 'car-repair-services' ),
		'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
	);
	echo '<ul class="post-meta">';
	echo '<li><span class="byline"> ' . $byline . '</span></li><li><i class="icon icon-clock"></i><span class="posted-on">' . $posted_on . '</span></li>'; // WPCS: XSS OK.
	echo '<li><i class="icon icon-interface"></i><span>';
	 comments_number( '0', '1', '%' );
	echo '</span></li>';
	echo '</ul>';
...
function car_repair_services_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links pull-left">' . esc_html__( '%1$s', 'car-repair-services' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
